...
const PlanDetailCard = () => {
    ...
    const { todaysPlan, setTodaysPlan, savedPlan, setSavedPlan, activeTab, sortBy } = context as FitLogContextType;

    const handleRemove = (exerciseId: number | string) => {
    if (activeTab === "today") {
        setTodaysPlan((prev) =>
    ...
    return (
        ...
        {sortedPlan.map((exercise) => (
            ...
            <div className="flex shrink-0 items-center gap-2">
                <Link href={`/FitLogDetails/${exercise.id}`}>
                <button
                    type="button"
                    className="
                    h-[31px]
